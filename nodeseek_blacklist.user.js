...
// @require      https://raw.githubusercontent.com/dajie111/nodeseek-userscript/refs/heads/main/topReply.js
// ==/UserScript==
...
(function () {
    ...
    // 新增：获取 拉黑/添加好友 按钮显示状态（默认开启）
    function getInteractionButtonsDisplayState() {
        return localStorage.getItem(INTERACTION_BUTTONS_DISPLAY_KEY) !== 'false';
    }
    ...
    // 新增：显示用户违规记录弹窗
    async function showViolationRecords(memberId, memberName) {
        memberName = memberName || '用户';
        ...
        document.addEventListener('mousemove', function(e) {
            ...
            box.style.height = 'calc(90vh - 20px)';
            overlay.style.padding = '0';
            overlay.style.alignItems = 'normal';
        });
        ...
        function formatDate(dateStr) {
            ...
            // 渲染记录列表
            function renderRecords(items, page, totalPages, total) {
                if (!items || items.length === 0) {
                    box.innerHTML = `
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-shrink:0">
                            <h3 style="margin:0">${escapeHtml(memberName)} 的违规记录</h3>
                            <button style="background:none;border:none;font-size:20px;cursor:pointer">&times;</button>
                        </div>
                        <div style="text-align:center;padding:40px 20px;color:#888;flex:1;display:flex;align-items:center;justify-content:center">暂无违规记录</div>
                    `;
                    box.querySelector('button').addEventListener('click', closeModal);
                    return;
                }

                let html = `
                    <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-shrink:0">
                        <h3 style="margin:0">${escapeHtml(memberName)} 的违规记录 (共 ${total} 条)</h3>
                        <button style="background:none;border:none;font-size:20px;cursor:pointer">&times;</button>
                    </div>
                    ...
                    <table style="width:100%;border-collapse:collapse;font-size:13px;table-layout:fixed">
                        <thead>
                            <tr style="background:#f3f4f6;position:sticky;top:0;z-index:1">
                                <th style="padding:8px;text-align:left;border-bottom:1px solid #e5e7eb;white-space:nowrap;width:60px">ID</th>
                                <th style="padding:8px;text-align:left;border-bottom:1px solid #e5e7eb;white-space:nowrap;width:100px">关联对象</th>
                                <th style="padding:8px;text-align:left;border-bottom:1px solid #e5e7eb">动作</th>
                                <th style="padding:8px;text-align:left;border-bottom:1px solid #e5e7eb;white-space:nowrap;width:80px">处理人</th>
                                <th style="padding:8px;text-align:left;border-bottom:1px solid #e5e7eb;white-space:nowrap;width:150px">时间</th>
                            </tr>
                        </thead>
                        <tbody>
                `;
                ...
                items.forEach(function(item) {
                    var postLink = '';
                    if (item.post_id && item.post_id !== -1) {
                        // 楼层锚点直接用 #{floor_index}
                        // NodeSeek 帖子每页 10 楼，自动计算页号：1-10→-1, 11-20→-2, ...
                        var floorIdx = (item.floor_index !== undefined && item.floor_index !== null && item.floor_index !== '') ? parseInt(item.floor_index, 10) : null;
                        var hasFloor = (floorIdx !== null && !isNaN(floorIdx) && floorIdx > 0);
                        var pageNum = hasFloor ? Math.floor((floorIdx - 1) / 10) + 1 : 1;
                        var postUrl = 'https://www.nodeseek.com/post-' + item.post_id + '-' + pageNum + (hasFloor ? '#' + floorIdx : '');
                        var linkText = hasFloor ? '评论' : '帖子';
                        var titleTip = hasFloor
                            ? '跳转到帖子 #' + item.post_id + ' 第 ' + floorIdx + ' 楼 (第 ' + pageNum + ' 页)'
                            : '跳转到帖子 #' + item.post_id;
                        postLink = '<a href="' + postUrl + '" target="_blank" title="' + titleTip + '" style="color:#2563eb">' + linkText + '</a>';
                    } else {
                        // 主页（蓝色链接），保持不动
                        postLink = '<a href="https://www.nodeseek.com/space/' + item.target_member_id + '#/general" target="_blank" style="color:#2563eb">主页</a>';
                    }

                    // 从 request_parsed 提取动作描述
                    var CATEGORY_MAP = {
                        promotion: '推广', inside: '内板', trade: '交易', info: '情报',
                        review: '测评', meaningless: '无意义', carpool: '拼车', daily: '日常',
                        tech: '技术', dev: 'dev', life: '生活', 'photo-share': '贴图',
                        expose: '曝光', sandbox: '沙盒',
                    };
                    var reqP = item.request_parsed || {};
                    var infos = [];
                    if (reqP.coin) {
                        if (reqP.coin.reason && reqP.coin.reason.trim()) {
                            infos.push(escapeHtml(reqP.coin.reason.trim()));
                        }
                        if (reqP.coin.coin_diff !== undefined && reqP.coin.coin_diff !== null && reqP.coin.coin_diff !== '') {
                            var cd = reqP.coin.coin_diff;
                            infos.push((cd > 0 ? '+' : '') + cd + ' 鸡腿');
                        }
                    }
                    if (reqP.suspend && reqP.suspend.status) {
                        var sd = reqP.suspend.value;
                        infos.push(sd ? '禁言 ' + sd + ' 天' : '禁言');
                    }
                    if (reqP.postSummary) {
                        var ps = reqP.postSummary;
                        if (ps.title) infos.push('标题被改为: ' + ps.title);
                        if (ps.locked === true) infos.push('锁定修改');
                        if (ps.locked === false) infos.push('解锁修改');
                        if (ps.category) infos.push('移动到' + (CATEGORY_MAP[ps.category] || ps.category) + '板块');
                        if (ps.award === true) infos.push('设为推荐阅读');
                        if (ps.status === true) infos.push('置顶评论');
                        if (ps.status === false) infos.push('取消置顶评论');
                        if (ps.rank !== undefined && ps.rank !== null) {
                            var rankLabel = ps.rank === 255 ? '（私有）' : '';
                            infos.push('阅读权限改为' + ps.rank + rankLabel);
                        }
                    }
                    if (reqP.hideComment && reqP.hideComment.status === true) {
                        infos.push('隐藏全部内容');
                    }
                    if (reqP.pinComment) {
                        if (reqP.pinComment.status === true) infos.push('置顶评论');
                        else if (reqP.pinComment.status === false) infos.push('取消置顶评论');
                    }
                    var actionDesc = infos.length > 0 ? infos.join(' | ') : '-';

                    html += '<tr style="border-bottom:1px solid #f0f2f5">' +
                        '<td style="padding:8px;white-space:nowrap">' + (item.id || '-') + '</td>' +
                        '<td style="padding:8px;white-space:nowrap">' + postLink + '</td>' +
                        '<td style="padding:8px;word-break:break-all">' + actionDesc + '</td>' +
                        '<td style="padding:8px;white-space:nowrap;color:#6b7280">' + escapeHtml(item.admin_member_name || '-') + '</td>' +
                        '<td style="padding:8px;color:#6b7280;font-size:12px;white-space:nowrap">' + formatDate(item.created_at) + '</td>' +
                        '</tr>';
                });
                ...
                // 分页
                if (totalPages > 1) {
                    html += '<div style="display:flex;gap:8px;align-items:center;margin-top:12px;justify-content:center">';
                    const prevDisabled = page <= 1;
                    const nextDisabled = page >= totalPages;
                    html += '<button class="vio-page-btn" data-page="' + (page - 1) + '" style="padding:4px 10px;border-radius:4px;border:1px solid #d1d5db;background:#fff;cursor:' + (prevDisabled ? 'not-allowed' : 'pointer') + ';opacity:' + (prevDisabled ? '0.4' : '1') + ';color:' + (prevDisabled ? '#999' : '#333') + '"' + (prevDisabled ? ' disabled' : '') + '>上一页</button>';
                    html += '<span style="font-size:13px;color:#6b7280">第 ' + page + ' / ' + totalPages + ' 页</span>';
                    html += '<button class="vio-page-btn" data-page="' + (page + 1) + '" style="padding:4px 10px;border-radius:4px;border:1px solid #d1d5db;background:#fff;cursor:' + (nextDisabled ? 'not-allowed' : 'pointer') + ';opacity:' + (nextDisabled ? '0.4' : '1') + ';color:' + (nextDisabled ? '#999' : '#333') + '"' + (nextDisabled ? ' disabled' : '') + '>下一页</button>';
                    html += '</div>';
                }
                ...
                // 绑定分页按钮
                box.querySelectorAll('.vio-page-btn').forEach(function(btn) {
                    btn.addEventListener('click', function() {
                        loadRecords(parseInt(this.dataset.page));
                    });
                });
            }
        ...
        box.innerHTML = `
            ...
            <div style="flex:1;display:flex;align-items:center;justify-content:center;color:#888;font-size:14px">数据加载中...</div>
            <style>@keyframes skeleton-shine{0%{background-position:200% 0}100%{background-position:-200% 0}}</style>
        `;
        box.querySelector('button').addEventListener('click', closeModal);
        ...
        async function loadRecords(page) {
            try {
                var apiUrl = 'https://hb.396663.xyz/api/ruling/by-member/' + memberId + '?page=' + page + '&pageSize=20';
                ...
                const data = await response.json();
                if (data.fetchDisabled) {
                    box.innerHTML = `
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-shrink:0">
                            <h3 style="margin:0">${escapeHtml(memberName)} 的违规记录</h3>
                            <button style="background:none;border:none;font-size:20px;cursor:pointer">&times;</button>
                        </div>
                        <div style="text-align:center;padding:40px 20px;flex:1;display:flex;flex-direction:column;align-items:center;justify-content:center;gap:8px">
                            <div style="font-size:48px;font-weight:700;color:#b91c1c;line-height:1">404</div>
                            <div style="font-size:14px;color:#6b7280">服务器连接失败，请联系插件作者。</div>
                        </div>
                    `;
                    box.querySelector('button').addEventListener('click', closeModal);
                } else if (data.success) {
                    renderRecords(data.items, data.page, data.totalPages, data.total);
                } else {
                    box.innerHTML = `
                        <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px;flex-shrink:0">
                            <h3 style="margin:0">${escapeHtml(memberName)} 的违规记录</h3>
                        ...
                        <div style="text-align:center;padding:40px 20px;color:#b91c1c;flex:1;display:flex;align-items:center;justify-content:center">加载失败: ${escapeHtml(data.message || '')}</div>
                    `;
                    box.querySelector('button').addEventListener('click', closeModal);
                }
            } catch (e) {
                box.innerHTML = `
                    ...
                    <div style="text-align:center;padding:40px 20px;color:#b91c1c;flex:1;display:flex;align-items:center;justify-content:center">网络错误</div>
                `;
                box.querySelector('button').addEventListener('click', closeModal);
            }
        }
    ...
    async function displayUserInfoFromData(userElement, userData) {
        ...
        recordBtn.textContent = '违规记录';
        recordBtn.className = 'user-violation-btn';
        ...
        recordBtn.addEventListener('click', function(e) {
            e.stopPropagation();
            showViolationRecords(userData.member_id, userData.member_name);
        });
    ...
    /* 私信 Markdown 图片点击放大 */
    html.ns-pm-talk-page .message-wrapper .markdown-content img,
    html.ns-pm-talk-page .message-wrapper .post-content img { cursor: zoom-in; }
    .ns-pm-lightbox { position: fixed; top: 0; left: 0; right: 0; bottom: 0; background: rgba(0,0,0,0.88); z-index: 2147483647; display: flex; align-items: center; justify-content: center; padding: 20px; box-sizing: border-box; }
    .ns-pm-lightbox-box { position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); max-width: 90vw; max-height: 90vh; display: flex; flex-direction: column; align-items: center; overflow: visible; }
    .ns-pm-lightbox-box img { max-width: 90vw; max-height: 80vh; object-fit: contain; border-radius: 4px; box-shadow: 0 4px 20px rgba(0,0,0,0.5); transition: transform 0.2s ease; transform-origin: center center; cursor: grab; }
    .ns-pm-lightbox-box img:active { cursor: grabbing; }
    .ns-pm-lightbox-controls { position: absolute; top: -44px; right: 0; display: flex; flex-wrap: nowrap; gap: 6px; z-index: 10; align-items: center; background: rgba(0,0,0,0.6); padding: 4px 8px; border-radius: 18px; box-shadow: 0 2px 8px rgba(0,0,0,0.4); transition: top 0.2s ease, right 0.2s ease; }
    .ns-pm-lightbox-controls button { min-width: 32px; height: 32px; padding: 0 6px; line-height: 30px; text-align: center; font-size: 16px; color: #fff; background: rgba(255,255,255,0.2); border: none; border-radius: 50%; cursor: pointer; white-space: nowrap; text-shadow: 0 1px 2px rgba(0,0,0,0.5); }
    .ns-pm-lightbox-controls button:hover { background: rgba(255,255,255,0.35); }
    .ns-pm-lightbox-controls a.ns-pm-lightbox-open-new { display: inline-flex; align-items: center; height: 28px; padding: 0 10px; color: #fff; background: rgba(255,255,255,0.2); border-radius: 14px; font-size: 12px; text-decoration: none; white-space: nowrap; line-height: 1; text-shadow: 0 1px 2px rgba(0,0,0,0.5); }
    .ns-pm-lightbox-controls a.ns-pm-lightbox-open-new:hover { background: rgba(255,255,255,0.35); }
    .ns-pm-lightbox-close { font-size: 22px !important; position: static !important; }
    .ns-pm-lightbox-footer { margin-top: 12px; color: rgba(255,255,255,0.7); font-size: 12px; text-align: center; max-width: 90vw; word-break: break-all; }
    @media (max-width: 768px) {
        .ns-pm-lightbox { padding: 10px; }
        .ns-pm-lightbox-box img { max-width: 95vw; max-height: 75vh; }
        .ns-pm-lightbox-controls { top: -40px; gap: 4px; padding: 3px 6px; }
        .ns-pm-lightbox-controls button { min-width: 28px; height: 28px; line-height: 26px; font-size: 14px; }
        .ns-pm-lightbox-controls a.ns-pm-lightbox-open-new { height: 24px; padding: 0 8px; font-size: 11px; }
    }
    ...
    function processUsernames() {
        ...
        document.querySelectorAll('a.author-name').forEach(function (a) {
            ...
            // 总是先把所有旧按钮隐藏起来，再统一处理；避免重复创建
            const existingBtns = parent.querySelectorAll('.' + SCRIPT_BUTTON_MARKER_CLASS);
            existingBtns.forEach(btn => { btn.style.display = 'none'; });

            // 如果关闭了「拉黑/添加好友按钮显示」，只保留高亮与备注显示，不再显示按钮
            if (!getInteractionButtonsDisplayState()) {
                a.style.whiteSpace = 'nowrap';
                return; // 按钮已隐藏，不重新创建
            }

            // 开关开启时复用已存在的按钮元素，只恢复显示，不重复创建
            if (existingBtns.length >= 2) {
                // 直接显示已有的两个按钮即可，保持 DOM 顺序不变
                existingBtns[0].style.display = '';
                existingBtns[1].style.display = '';
                a.style.whiteSpace = 'nowrap';
                return;
            }
        ...
        const obs = new MutationObserver(function(mutations) {
            for (const m of mutations) {
                ...
                for (const node of m.addedNodes) {
                    ...
                    if (node.id && /^nodeseek-plugin|blacklist-dialog|friends-dialog|browse-history-dialog|logs-dialog/.test(node.id)) continue;
                    if (node.closest && node.closest('#nodeseek-plugin-container, #blacklist-dialog, #friends-dialog, #browse-history-dialog, #logs-dialog')) continue;
                    // 立即同步执行，不 debounce
    ...
    /** 获取 NS 图床分类与置顶设置（通过 localStorage 读取，适配 nodeImage.js 的存储键） */
    function getNsImageSettings() {
        var settings = {};
        try {
            var cats = localStorage.getItem('ns_ni_categories');
            if (cats) {
                var parsed = JSON.parse(cats);
                if (Array.isArray(parsed)) settings.categories = parsed;
            }
        } catch (e) { console.error('导出NS图床分类失败:', e); }
        try {
            var map = localStorage.getItem('ns_ni_img_cats');
            if (map) {
                var parsed = JSON.parse(map);
                if (typeof parsed === 'object') settings.imageCatMap = parsed;
            }
        } catch (e) { console.error('导出NS图床图片分类映射失败:', e); }
        try {
            var pins = localStorage.getItem('ns_ni_pins');
            if (pins) {
                var parsed = JSON.parse(pins);
                if (Array.isArray(parsed)) settings.pinnedIds = parsed;
            }
        } catch (e) { console.error('导出NS图床置顶失败:', e); }
        return settings;
    }

    /** 导入 NS 图床分类与置顶设置 */
    function importNsImageSettings(settings) {
        if (!settings || typeof settings !== 'object') return;
        try {
            if (Array.isArray(settings.categories)) {
                localStorage.setItem('ns_ni_categories', JSON.stringify(settings.categories));
            }
            if (typeof settings.imageCatMap === 'object') {
                localStorage.setItem('ns_ni_img_cats', JSON.stringify(settings.imageCatMap));
            }
            if (Array.isArray(settings.pinnedIds)) {
                localStorage.setItem('ns_ni_pins', JSON.stringify(settings.pinnedIds));
            }
            if (typeof GM_setValue === 'function') {
                try {
                    if (Array.isArray(settings.categories)) GM_setValue('ns_ni_categories', settings.categories);
                    if (typeof settings.imageCatMap === 'object') GM_setValue('ns_ni_img_cats', settings.imageCatMap);
                    if (Array.isArray(settings.pinnedIds)) GM_setValue('ns_ni_pins', settings.pinnedIds);
                } catch (e) { console.error('导入NS图床设置到GM失败:', e); }
            }
        } catch (e) { console.error('导入NS图床设置失败:', e); }
    }
    ...
    function exportBlacklist() {
        ...
        const nsImageSettings = getNsImageSettings(); // NS图床分类与置顶设置
        ...
        const data = JSON.stringify({
            ...
            nsImageSettings: nsImageSettings
        }, null, 2);
        ...
        exportDesc += '、设置';
        // NS图床设置
        if (nsImageSettings && Object.keys(nsImageSettings).length > 0) {
            exportDesc += '、NS图床设置';
        }
        exportDesc += ')';
        addLog(exportDesc);
    ...
    // 新增：处理屏蔽URL跳转提醒设置
                    ...
                    // 导入 NS 图床分类与置顶设置
                    if (json.nsImageSettings && typeof json.nsImageSettings === 'object') {
                        importNsImageSettings(json.nsImageSettings);
                        var nsCatCount = Array.isArray(json.nsImageSettings.categories) ? json.nsImageSettings.categories.length : 0;
                        var nsPinCount = Array.isArray(json.nsImageSettings.pinnedIds) ? json.nsImageSettings.pinnedIds.length : 0;
                        if (nsCatCount > 0 || nsPinCount > 0) {
                            importDesc += '、NS图床设置';
                        }
                    }
                    importDesc += ')';
    ...
    function addExportImportButtons() {
        ...
        favoriteBtn.className = 'blacklist-btn';
        favoriteBtn.style.background = '#2196F3'; // 蓝色背景
        favoriteBtn.textContent = '收藏';
        favoriteBtn.style.flex = '1';
        favoriteBtn.onclick = function () {
            if (window.NodeSeekCollect && typeof window.NodeSeekCollect.addCurrentPageToFavorites === 'function') {
                window.NodeSeekCollect.addCurrentPageToFavorites();
            } else {
                alert('收藏功能未加载');
            }
        };
        ...
        viewFavBtn.id = 'view-fav-btn';
        viewFavBtn.className = 'blacklist-btn';
        viewFavBtn.style.background = '#2ea44f'; // 绿色背景
        viewFavBtn.textContent = '查看';
        viewFavBtn.style.flex = '1';
        viewFavBtn.onclick = function () {
            if (window.NodeSeekCollect && typeof window.NodeSeekCollect.showFavoritesDialog === 'function') {
                window.NodeSeekCollect.showFavoritesDialog();
            } else {
                alert('收藏查看功能未加载');
            }
        };
        ...
        favoritesContainer.style.marginTop = '1px';
        favoritesContainer.appendChild(favoriteBtn);
        favoritesContainer.appendChild(viewFavBtn);
        ...
        hotTopicsBtn.id = 'ns-hot-topics-btn';
        hotTopicsBtn.className = 'blacklist-btn';
        hotTopicsBtn.style.background = '#e91e63';
        hotTopicsBtn.textContent = '热点统计';
        hotTopicsBtn.style.width = '100%';
        hotTopicsBtn.style.marginTop = '1px';
        ...
        const hotTopicsBtnContainer = document.createElement('div');
        hotTopicsBtnContainer.style.display = 'flex';
        hotTopicsBtnContainer.style.flexDirection = 'row';
        hotTopicsBtnContainer.style.gap = '10px';
        hotTopicsBtnContainer.style.width = '100%';
        hotTopicsBtnContainer.appendChild(hotTopicsBtn);
        ...
        container.appendChild(favoritesContainer); // 收藏和查看
        ...
        container.appendChild(hotTopicsBtnContainer); // 热点统计
    ...
    function showSettingsDialog() {
        ...
        // 2. 拉黑/好友按钮显示开关
        const interactionBtnRow = document.createElement('div');
        interactionBtnRow.style.display = 'flex';
        interactionBtnRow.style.justifyContent = 'space-between';
        interactionBtnRow.style.alignItems = 'center';
        if (isMobile) interactionBtnRow.style.flexWrap = 'wrap';

        const interactionBtnLabel = document.createElement('label');
        interactionBtnLabel.textContent = '显示拉黑/好友按钮';
        interactionBtnLabel.style.fontWeight = '500';
        interactionBtnLabel.style.color = '#555';

        const interactionBtnSwitch = document.createElement('input');
        interactionBtnSwitch.type = 'checkbox';
        interactionBtnSwitch.checked = getInteractionButtonsDisplayState();
        interactionBtnSwitch.style.transform = 'scale(1.2)';
        interactionBtnSwitch.onchange = function() {
            const newState = this.checked;
            setInteractionButtonsDisplayState(newState);
            // 立刻刷新：重新注入/移除按钮
            processUsernames();
            addLog('拉黑/好友按钮显示：' + (newState ? '开启' : '关闭'));
        };

        interactionBtnRow.appendChild(interactionBtnLabel);
    ...
    // ====== 私信 Markdown 图片点击放大 ======
    (function initPrivateMessageImageZoom() {
        const LIGHTBOX_ID = 'ns-pm-image-lightbox';
        const TALK_PAGE_CLASS = 'ns-pm-talk-page';
        let dragState = null;
        ...
        function isPrivateMessageTalkPage() {
            if (!isNotificationPage()) return false;
            const hash = window.location.hash || '';
            return /[?&]mode=talk\b/.test(hash);
        }

        function updateTalkPageClass() {
            const root = document.documentElement;
            if (!root) return;
            root.classList.toggle(TALK_PAGE_CLASS, isPrivateMessageTalkPage());
        }

        function closeLightbox() {
            const box = document.getElementById(LIGHTBOX_ID);
            if (box) box.remove();
            dragState = null;
        }
        ...
        document.addEventListener('mousemove', function(e) {
            ...
            dragState.box.style.left = Math.max(0, dragState.initialLeft + dx) + 'px';
            dragState.box.style.top = Math.max(0, dragState.initialTop + dy) + 'px';
        });
        ...
        function openLightbox(src) {
            ...
            overlay.className = 'ns-pm-lightbox';
            overlay.innerHTML = `
                <div class="ns-pm-lightbox-box">
                    <div class="ns-pm-lightbox-controls">
                        <a class="ns-pm-lightbox-open-new" href="${escapeHtml(src)}" target="_blank" rel="noopener noreferrer" title="${escapeHtml(src)}">新窗口打开</a>
                        <button class="ns-pm-zoom-in" title="放大">+</button>
                        <button class="ns-pm-zoom-out" title="缩小">−</button>
                        <button class="ns-pm-zoom-reset" title="还原图片到初始位置和大小">1:1</button>
                        <button class="ns-pm-lightbox-close" title="关闭">×</button>
                    </div>
            ...
            const box = overlay.querySelector('.ns-pm-lightbox-box');
            const img = overlay.querySelector('img');
            const controls = overlay.querySelector('.ns-pm-lightbox-controls');
            const closeBtn = overlay.querySelector('.ns-pm-lightbox-close');
            const zoomInBtn = overlay.querySelector('.ns-pm-zoom-in');
            const zoomOutBtn = overlay.querySelector('.ns-pm-zoom-out');
            const zoomResetBtn = overlay.querySelector('.ns-pm-zoom-reset');

            let currentScale = 1;
            const MIN_SCALE = 0.5;
            const MAX_SCALE = 5;
            const ZOOM_STEP = 1.25;
            const CONTROLS_BASE_TOP = -44; // 控件初始 top 值（CSS 中定义）

            function applyZoom() {
                img.style.transform = 'scale(' + currentScale.toFixed(2) + ')';
                // 控件不缩放，仅跟随图片缩放按比例移动位置
                const imgHeight = img.offsetHeight;
                const imgWidth = img.offsetWidth;
                if (imgHeight > 0) {
                    const vOffset = (currentScale - 1) * imgHeight / 2;
                    controls.style.top = (CONTROLS_BASE_TOP - vOffset) + 'px';
                }
                if (imgWidth > 0) {
                    const hOffset = (currentScale - 1) * imgWidth / 2;
                    controls.style.right = (-hOffset) + 'px';
                }
            }
            ...
            // ESC 关闭图片
            function onEsc(e) {
                if (e.key === 'Escape' && document.getElementById(LIGHTBOX_ID)) {
                    closeLightbox();
        ...
        // 全局事件委托：仅私信对话页生效
        document.addEventListener('click', function(e) {
            if (!isPrivateMessageTalkPage()) return;
            const img = e.target.closest('.message-wrapper .markdown-content img, .message-wrapper .post-content img');
            if (!img) return;
        ...
        window.addEventListener('hashchange', function() {
            updateTalkPageClass();
            closeLightbox();
            setTimeout(updateTalkPageClass, 150);
            setTimeout(updateTalkPageClass, 400);
        });
        ...
        updateTalkPageClass();
        setTimeout(updateTalkPageClass, 300);
        setTimeout(updateTalkPageClass, 1500);
    })();
